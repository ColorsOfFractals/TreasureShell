...
function Find-ISCC {
    ...
    # 2. Ask Windows where Inno Setup registered itself
    $RegistryRoots = @(
    ...
    # 3. Known user/machine fallbacks
    $Candidates = @(
...
if (-not $ISCC) {
    ...
    # Installation may be per-user or per-machine.
    # Ask Windows again instead of assuming a folder.
    $ISCC = Find-ISCC
}
...
# Verify actual compiler capability.
$PreviousErrorActionPreference = $ErrorActionPreference
$ErrorActionPreference = "Continue"
...
if ($ISCCExit -ne 0) {
    throw "ISCC.exe was found but failed capability verification."
}
